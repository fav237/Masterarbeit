...
class CarEnv:
    ...
    def step(self, action):
        ...
        return image/255, self.state(), reward, done, None
    ...
    def observation(self, image):
        ...
        for wp in (self.waypoints_route):
            
            distance = math.sqrt((wp[1] - pos_y)**2 +(wp[0] -pos_x)**2)
            angle = math.degrees(math.atan2(wp[1] -pos_y, wp[0] -pos_x)) -vehicle_yaw
            distance_wp = math.sqrt((wp[1] - current_waypoint[1])**2 +(wp[0] -current_waypoint[0])**2)
            ...
            if abs(angle)<=90:
                points_ahead.append([distance,angle])
            else:
                points_behind.append([distance,angle])
            if len(points_ahead)==0:
                closest = min(points_behind, key=lambda x: x[0])
                if closest[1]>0:
                    closest = [closest[0],90]
                else:
                    closest = [closest[0],-90] 
            else:
                closest = min(points_ahead, key=lambda x: x[0])
                # move forward if too close
                for  point in points_ahead:
                    if point[0]>=10 and point[0]<20:
                        closest = point
                        break
            # now we pick a point we need to get angle to

            # check the direction 
            if distance_wp >= 10.0 and distance_wp <= 20.0:
                angle_wp = ((wp[2]%360) -(current_waypoint[2]%360))%180
        ...
        self.data['angle'] = closest[1]/90.0
        self.data['lateral_dist_t'] = lateral_distance
        angle = np.array(closest[1]/90.0).reshape((1, ))
    ...
    def state(self):
        ...
        return: array of size[9,], torch.Tensor (v_x, v_y, a_x, a_y
                                                 ...
                                                 future_angles)
        '''
        velocity_t = (self.data['velocity_t']-20)/20
        accel_t = (self.data['acc_t']-20)/20
