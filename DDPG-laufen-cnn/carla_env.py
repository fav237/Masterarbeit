...
class CarEnv(gym.Env):
    ...
    def reset(self, relaunch=False):
        ...
        if self.train_mode == self.train[0]:
            self.pos_a = carla.Transform(carla.Location(x=-10, y=45.0, z=1.8),carla.Rotation(pitch=0.0, yaw=92.0, roll=0.0))
            self.pos_b = carla.Location(x=-10, y=180, z=1.8)
        ...
        self.sem_cam_bp.set_attribute('image_size_x', f'{self.im_width_cnn}')
        self.sem_cam_bp.set_attribute('image_size_y', f'{self.im_height_cnn}')
        self.sem_cam_bp.set_attribute('fov', '90')
        self.sem_cam_bp.set_attribute('sensor_tick', '0.02')
        sem_transform = carla.Transform(carla.Location(z=self.cam_z+0.4,x=self.cam_x))
        ...
        image_sem = self.front_camera_sem
        self.observation(image)

        #self.settings.synchronous_mode = True
        #self.world.apply_settings(self.settings)

        return  [image_sem, self.state()]
    ...
    def sem_cam_img(self, image):
        ...
        image = image.reshape((self.im_height_cnn, self.im_width_cnn, 4))
        image = image[:, :, :3]
    ...
    def step(self, action):
        ...
        return [image_sem, self.state()], reward, done, self.data
    ...
    def state(self):
        ...
        ## camera image
        sem_im = self.front_camera_sem/255
        ...
        return [sem_im, np.float32(info_vec)]
