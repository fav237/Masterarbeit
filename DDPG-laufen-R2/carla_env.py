...
class CarEnv(gym.Env):
    ...
    def __init__(self):
        ...
        self.observation_space = spaces.Box(low=-1.0, high=1.0, shape=(4,0 ))
    ...
    def reset(self, relaunch=False):
        ...
        if self.train_mode == self.train[2]:
            while self.total_dist < 50 and self.dif_angle_routes == 0:
                self.pos_a = self.pos_a = carla.Transform(carla.Location(x=20.75, y=302.5, z=0.6),carla.Rotation(pitch=0.0, yaw=-180.0, roll=0.0))
        ...
        #self.draw_waypoint( self.current_route, tl=self.line_time)
    ...
    def draw_waypoint(self, current_plan, tl):
        for waypoint in current_plan:
            self.world.debug.draw_string(waypoint[0].transform.location, '°', draw_shadow=False,
            color=red, life_time=tl,
    ...
    def step(self, action):
        ...
        #time.sleep(0.01)
        #reward for steering
        angle =self.data['delta_yaw_t']#*np.pi/180
        trackpos = self.data['lateral_dist_t']
        print(f'angle:{angle}')
       # reward += 10 *(np.cos(angle))
        # if v.x < 10:
        #     reward += int(10*((np.cos(angle)) -np.abs(np.sin(angle)) - np.abs(trackpos)))
        # else:
    ...
    def observation(self, image):
        self.distances = [ 0.5, 0.75, 1.]   # 1 and 2
        angles =[]
