    ...
    def trainAgent(self):
        ...
        for i in range(self.episodes_num):
            ...
            for j in range(self.max_steps):
                ...
                noise[0][0] =  max(epsilon, 0) * self.ou.function(action[0][0],  0.0 , 0.50, 0.10)
                #if action[0][1] >=0 :
            ...
            ep_avg_speed.append(avg_speed)
            if (i > 0) and ((i % self.AGGREGATE_STATS_EVERY == 0) or (i ==1)):
                #reward = (ep_rewards[-self.AGGREGATE_STATS_EVERY:])
                ...
                average_speed = np.mean(ep_avg_speed[-self.AGGREGATE_STATS_EVERY:])
                self.tensorboard.update_stats(average_reward=average_reward, min_reward=min_reward, max_reward=max_reward,
                                     average_speed=average_speed, distance=average_dist, loss=loss)
            
            
            #Save training data to files
            if i % 10 == 0 and (total_reward > 50):
