...
class DDPGAgent:
    ...
    def __init__(self):
        ...
        self.critic_weights_file = "data/datacritic.h5"
        self.save_weights_path = "data/" 
        self.N_save_stats = 50
    
        # TRAIN PARAMETERS
        self.tau = 0.001                # Target Network HyperParameter
        self.lra = 0.0001               # 0.0001       # Learning rate for Actor
        self.lrc = 0.001                # 0.001        # Learning rate for Critic
        ...
        self.explore = 97000            # 96000., 100000
        self.epsilon = 1.0
        self.epsilon_min = 0.1
        self.step = 0
        self.hidden_units=[350,650,650]
        #self.hidden_units=[256,512,512]
    ...
    def trainAgent(self):
        ...
        for i in range(self.episodes_num):
            ...
            print("Episode : %s Replay buffer %s" % (i, str(buffer.count())))
            if np.mod(i, 15) == 0:
            ...
            for j in range(self.max_steps):
                ...
                noise[0][0] =  max(epsilon, 0) * self.ou.function(action[0][0],  0.0 , 0.5, 0.22)
                noise[0][1] =  max(epsilon, 0) * self.ou.function(action[0][1],  0.40 , 1.0, 0.1)
                ...
                buffer.add((state, action_noise[0], reward, new_state, done))  

                # batch update
                batch = buffer.getBatch(self.BATCH_SIZE)

                # build arrays for models from batch
                ...
                actor_target = self.actor.target_model.predict(new_states)
                target_q_values = self.critic.target_model.predict([new_states, actor_target])
